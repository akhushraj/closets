...
export const DEFAULTS = {
  esPlan: "36, 36", esTop: 84, depth: 24, fronts: "7, 8, 9", rodZ: 62, esShelf: 78,
  ...shelfSlots("s", [30, 44, 58, 72], [16, 86]),
  aboveOn: true, aboveZ: 92, drawersOut: false,
...
export function build(p) {
  ...
  // one plywood shelf right across, over the boxes, tying the three together
  if (p.aboveOn) add(fixedShelves(w.T, { u0: 0, u1: W, depth: d, levels: [p.aboveZ], label: "Shelf over the boxes" }));
  ...
  return {
    ...
    stats: [
      ...
      { k: "Shelf across", v: p.aboveOn ? frac(p.aboveZ, 8) : "off", s: p.aboveOn ? `full ${frac(W, 8)}, over the boxes` : "" },
    ],
    titleMeta: [
    ...
    gcText: [
      ...
      `AMIR: 3/4" birch plywood shelves on 1x2 cleats into the studs, ${frac(endW, 8)} wide at each end, ${frac(d, 8)} deep, at ${lv.map(z => frac(z, 8)).join(", ")}.${p.aboveOn ? ` Plus one shelf right across the full ${frac(W, 8)} at ${frac(p.aboveZ, 8)}, over the boxes.` : ""}`,
